...
def organize_newGreedy_plot(): #整理所有的关于“使用第一个ses的mask”以及“使用前一个ses的数据获得的mask”的比较画图，包括了4way的均值，2way的均值。AB CD AC AD 的均值变化情况。
    ...
    def getAB_acc_ses1_leaveout(logID='17927715',axis="AB"):
    ...
    for CurrLeaveOut in ses1Log_dict:
        t = getAB_acc_ses1_leaveout(logID=ses1Log_dict[CurrLeaveOut],axis="AB")
        AB.append(t)
    CD=[]
    for CurrLeaveOut in ses1Log_dict:
        t = getAB_acc_ses1_leaveout(logID=ses1Log_dict[CurrLeaveOut],axis="CD")
        CD.append(t)

    AC=[]
    for CurrLeaveOut in ses1Log_dict:
        t = getAB_acc_ses1_leaveout(logID=ses1Log_dict[CurrLeaveOut],axis="AC")
        AC.append(t)
        
    AD=[]
    for CurrLeaveOut in ses1Log_dict:
        t = getAB_acc_ses1_leaveout(logID=ses1Log_dict[CurrLeaveOut],axis="AD")
        AD.append(t)
